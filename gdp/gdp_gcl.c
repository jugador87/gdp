...
void
gdp_gcl_msg_print(const gdp_msg_t *msg, FILE *fp)
{
...
{
	EP_STAT estat = ep_b64_encode(internal, sizeof(gcl_name_t), external,
	        sizeof(gcl_pname_t),
	        EP_B64_ENC_URL);
	...
	if (!EP_STAT_ISOK(estat))
	{
		...
		ep_dbg_cprintf(Dbg, 2, "gdp_gcl_printable_name: ep_b64_encode failure\n"
...
gdp_gcl_internal_name(const gcl_pname_t external, gcl_name_t internal)
{
	...
	{
		estat = ep_b64_decode(external, sizeof(gcl_pname_t) - 1, internal,
		        sizeof(gcl_name_t),
		        EP_B64_ENC_URL);
	}
	...
	if (!EP_STAT_ISOK(estat))
	{
		...
		ep_dbg_cprintf(Dbg, 2, "gdp_gcl_internal_name: ep_b64_decode failure\n"
				"\tstat = %s\n", ep_stat_tostr(estat, ebuf, sizeof ebuf));
	}
...
static EP_STAT
ack_success(gdp_pkt_hdr_t *pkt, gcl_handle_t *gclh, struct bufferevent *bev,
...
static EP_STAT
nak_client(gdp_pkt_hdr_t *pkt, gcl_handle_t *gclh, struct bufferevent *bev,
        struct gdp_conn_ctx *conn)
{
...
static EP_STAT
nak_server(gdp_pkt_hdr_t *pkt, gcl_handle_t *gclh, struct bufferevent *bev,
        struct gdp_conn_ctx *conn)
{
...
typedef EP_STAT
cmdfunc_t(gdp_pkt_hdr_t *pkt, gcl_handle_t *gclh, struct bufferevent *bev,
        struct gdp_conn_ctx *conn);
...
gdp_init(bool run_event_loop)
{
	...
	bev = bufferevent_socket_new(GdpEventBase, -1,
	...
	else
	{
		...
		if (bufferevent_socket_connect(bev, (struct sockaddr *) &sa, sizeof sa)
		        < 0)
		{
...
gcl_handle_new(gcl_handle_t **pgclh)
{
	...
	*pgclh = gclh;
	...
	fail1: estat = ep_stat_from_errno(errno);
	return estat;
}
...
EP_STAT
gdp_gcl_print(const gcl_handle_t *gclh, FILE *fp, int detail, int indent)
{
	gcl_pname_t nbuf;
...
EP_STAT
gdp_gcl_create(gcl_t *gcl_type, gcl_name_t gcl_name, gcl_handle_t **pgclh)
{
...
EP_STAT
gdp_gcl_open(gcl_name_t gcl_name, gdp_iomode_t mode, gcl_handle_t **pgclh)
{
	EP_STAT estat = EP_STAT_OK;
	gcl_handle_t *gclh = NULL;
	int cmd;
	...
	*pgclh = gclh;
...

	{
		...
		ep_dbg_cprintf(Dbg, 10, "Couldn't open GCL %s: %s\n", pname,
		        ep_stat_tostr(estat, ebuf, sizeof ebuf));
	}
	return estat;
}
...
EP_STAT
gdp_gcl_append(gcl_handle_t *gclh, gdp_msg_t *msg)
{
	EP_STAT estat;
...
EP_STAT
gdp_gcl_read(gcl_handle_t *gclh, long msgno, gdp_msg_t *msg,
