...
// TBD: Needed - we should be statless wrt GDP, right? gdp_rest_init( '' );
...
function(req, res)
{
	...
	// gdp_init_js();  // this isn't the place for it but this works here
	res.json( get_gcl_base64_namesSync() );
});
...
function(req, res)
{
	...
	// TBD: check for existence/non-existence.  Can this be done with the
	//      current libgdp?
	// TBD: awaiting libgdp additions for GCL metadata
...
function(req, res)
{
	...
	*/
	...
	{
		wrv = 
		{ error_isok: true, error_code: 0, error_msg: "benign error_msg",
		  gcl_name: gcl_name
		};
	}
	...
	// LEFT OFF - where do these come from?
...
function(req, res)
{
	...
	var res_body = "";  // will hold opaque data from record recno
	if (false)
	{
	var res_body = 
	{	// Currently, no metadata other than the name
...
//  Modified to:
//D GET /gdp/v1/gcl/<gcl_name>/<recno#>/<nrecs#> — return a specified record
//  TBD: check for lexical incompatibilities with URL-encoded constraints
//D Request Data
//D none
//D Response
...
//D GET /gdp/v1/gcl/gcl_id?recno=<#>&nrecs=<#>&timeout=<seconds> — monitor a GCL
//D Request Data
//D none
//D Response Data
//D 200 OK
...
//D Description
//D If the indicated GCL does not exist, the GET returns immediately with a 404 error code. Otherwise, if the indicated record number exists, the result is exactly the same as the previous case. If the indicated record number does not exist, this call waits for up to the indicated timeout for that record to appear; if it does, the record is returned in the usual way, otherwise it returns with a 408 Request Timeout response. If the starting record number is not specified, it starts from the beginning.
//D [[Perhaps this should not take recno and nrecs, and just return the next record that appears at the GCL.]]
//D [[There is some debate about whether this is the correct interface rather than falling back to a non-REST interface (such as WebSockets) for subscriptions. HTTP (and hence REST) isn’t designed to handle spontaneous server to client messages.]]
...
// For now, this is by a ^C from the terminal it was started in.
