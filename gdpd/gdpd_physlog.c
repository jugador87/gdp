...
static EP_STAT
gcl_handle_save_offset(gcl_handle_t *gclh, gdp_msgno_t msgno, off_t off)
{
	...
	if (off != gclh->offcache[msgno] && gclh->offcache[msgno] != 0)
	{
		...
		gdp_log(EP_STAT_ERROR,
		        ...
		        msgno, (long long) gclh->offcache[msgno], (long long) off);
...
static EP_STAT
get_gcl_rec(FILE *fp, gdp_msg_t *msg, struct evbuffer *evb)
{
	long offset;
	EP_STAT estat = EP_STAT_OK;
...

	{
		...
		tx = bx = bstart = bend = 0;
		offset = ftell(fp);
		ep_dbg_cprintf(Dbg, 50, "get_gcl_rec: initial offset %ld\n", offset);
...
EP_STAT
gcl_read(gcl_handle_t *gclh, gdp_msgno_t msgno, gdp_msg_t *msg,
        struct evbuffer *evb)
{
	EP_STAT estat = EP_STAT_OK;
	...
	// ok, done!
	fail0: flock(fileno(gclh->fp), LOCK_UN);
...
gcl_create(gcl_name_t gcl_name, gcl_handle_t **pgclh)
{
	...
	{
		estat = ep_stat_from_errno(errno);
		gdp_log(estat, "gcl_create: cannot create %s: %s", pbuf,
		        strerror(errno));
		goto fail1;
	}
	...
	{
		estat = ep_stat_from_errno(errno);
		gdp_log(estat, "gcl_create: cannot fdopen %s: %s", pbuf,
		        strerror(errno));
		goto fail1;
	}
...
gcl_append(gcl_handle_t *gclh, gdp_msg_t *msg)
{
	...
	ep_dbg_cprintf(Dbg, 49, "Back from locking for message %d, cachesize %ld\n",
	...
	estat = gcl_handle_save_offset(gclh, ++gclh->msgno, ftell(gclh->fp));
	EP_STAT_CHECK(estat, goto fail0);
...
static void
gcl_sub_event_cb(evutil_socket_t fd, short what, void *cbarg)
{
	gdp_msg_t msg;
	EP_STAT estat;
...
EP_STAT
gcl_subscribe(gcl_handle_t *gclh, gdp_gcl_sub_cbfunc_t cbfunc, long offset,
        void *buf, size_t bufsiz, void *cbarg)
{
	EP_STAT estat = EP_STAT_OK;
