...
void
write_scgi(scgi_request *req, char *sbuf)
{
	int dead = 0;
	int i;
...
EP_STAT
show_gcl(char *gclpname, scgi_request *req)
{
...
read_msg(char *gclpname, long msgno, scgi_request *req)
{
	...
	fail0:
	{
		...
		gdp_failure(req, "404", "Cannot read GCL", "ss", "GCL", gclpname,
...
EP_STAT
process_scgi_req(scgi_request *req, short what, void *arg)
{
	...
	if (*gclpname == '\0')
	{
		if (req->request_method == SCGI_METHOD_POST)
		{
			...
			if (EP_STAT_ISOK(estat))
			{
				...
				snprintf(sbuf, sizeof sbuf, "HTTP/1.1 201 GCL created\r\n"
						"Content-Type: application/json\r\n"
						"\r\n"
						"{\r\n"
						"    \"gcl_name\": \"%s\"\r\n"
	...
	else if (*uri == '\0')
	{
		...
		else if (req->request_method == SCGI_METHOD_POST)
		{
			...
			if (!EP_STAT_ISOK(estat))
			{
				...
				gdp_failure(req, "420" "Cannot append to GCL", "ss", "GCL",
				        gclpname, "error",
			...
			// success: send a response
			{
				...
				fprintf(fp, "HTTP/1.1 200 Successfully appended\r\n"
						"Content-Type: application/json\r\n"
						"\r\n"
						"{\r\n"
						"    \"msgno\": \"%d\"", msg.msgno);
	...
	error404: estat = gdp_failure(req, "404", "Unknown URI or method", "ss",
	        "uri", uri, "method", scgi_method_name(req->request_method));
	return estat;

	error500: estat = gdp_failure(req, "500", "Internal Server Error", "s",
...
main(int argc, char **argv, char **env)
{
	...
	{
		ep_app_error("could not initialize SCGI port %d: %s", listenport,
		        strerror(errno));
		return EX_OSERR;
	}
